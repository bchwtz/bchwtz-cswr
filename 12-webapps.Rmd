...
For the creation of the elements 2.--5. we use the functions listed under *Inputs* (see [@shinyCheatsheet, p. 1]):

element of the UI        | implementation in `shiny`
-------------------------|---------------------------
...
The code presented does not work since several arguments of the single functions have not been defined so far (see the description of the single functions and @shinyCheatsheet).  Each *Input*- and *Output*-function needs an *Id* to address the corresponding element (e.g. to access values). [@shinyCheatsheet]

Furthermore, 

+ we have to set the values of the single *Inputs* (e.g. from which range of values can the user choose one),
...
+ "N", "n", "showdensity" and "seed" (the first argument of the Input functions) are the single ids,
...
In the server function, we can also access values of one of the inputs on the ui by calling `input$Id_Input` where `Id_Input` is the id of the element we have defined as input, e.g. `seed` is the id the numeric Input where the user sets a number for a random seed [@shinyCheatsheet, p. 1]:
...
Finally, running the app leads to the result illustrated in the gif in the beginning of this chapter.
