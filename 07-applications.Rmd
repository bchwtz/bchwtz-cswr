...
As we have seen in chapter \@ref(plotsbaseR), the `airquality` data set consists von values from May to September in 1973. [@R] How can we create several boxplots of the temperature for the single months in one plot?
...
1) By creating a factor that consists of the categories "May", "June", "July", "August" and "September" (saved as varibale `Month_categ`)
...
2) A shorter version is to use the argument `group` inside `aes()`
...
Remember the definition of linear models in base-R based on function `lm()` as we have already done in chapter \@ref(plotsbaseR):
...
The following examples are based on the `airquality` data set [@R] as well as on figure \@ref(fig:first-ggplot-air).
...
(ref:ggplot1-air-p1-2) Scatterplot of wind speed vs. temperature of the `airquality` data set [@R] with triangular points.
...
(ref:ggplot1-air-p1-3) Scatterplot of wind speed vs. temperature of the `airquality` data set [@R] with triangular points.
...
(ref:ggplot1-air-p2) Another scatterplot of wind speed vs. temperature of the `airquality` data set [@R] with modified points.

```{r first-ggplot-air-points3, fig.cap="(ref:ggplot1-air-p3)"}
ggplot(data = airquality, mapping = aes(x = Temp, y = Wind))+
  geom_point(alpha = 0.5, shape = 21, color = "grey", fill = "cyan", size = 3, position = "jitter")+
  geom_text(mapping = aes(label = ifelse(Temp > 80, Temp, "")))+
  ggtitle("Scatterplot of temperature and wind speed")+ 
xlab("Temperature [°F]")+ ylab("Wind [mph]")
```
...
+ the argument `shape` controls the shape of the plotting symbols (see `?points()` for more information).

+ the arguments `color` and `fill` control the color of the symbols dependent on the shape of the symbols (i.e. whether the symbols have a border or not).
...
+ text is added to the plot via `geom_text(maping = aes(label = ...))`.

+ `position = "jitter"` adds random noise to the plotting symbols to avoid overplotting.


**Changing scales**

Here are some exmaples how scales can be changed in `ggplot2`; for further information see @ggplot2Cheatsheet.

```{r}
ggplot(data = airquality, mapping = aes(x = (Temp-32)*5/9, y = Wind))+ 
  # modify the temperature from degree Fahrenheit to Celsius
# (National Institute of Standards and Technology (NIST) (2021))
  geom_point()+
  ggtitle("Scatterplot")+ xlab("Temperature [°C]")+ ylab("Wind [mph]")
```
...
The modification of legends is illustrated based on figure \@ref(fig:ggplot-month-categ) in which we created boxplots of the temperature for each month. Now, we color the boxplots (one color for each month) by setting `fill = Month_categ`). A legend has been automatically added:
...
Since the previous plot is saved as `boxplot_Temp_Month` some more specifications can be easily added to it without repeating every line of code (i.e. the single layers).
...
From figure \@ref(fig:boxplot-legend1) and its corresponding code it is obvious that

+ the legend's characteristics are accessed via `legend.*` inside `theme()`.
...
As it was announced in the beginning of this chapter \@ref(plotsggplot2) `ggplot2` provides help in dealing with more than two variables. The simplest way is to use the arguments of `aes()` in order to color, group (see figure \@ref(fig:ggplot-month-categ)),... values, e.g.:

```{r ggpot-more2var-1, fig.cap="(ref:more2var-1)"}
...
We can simply define multiple arguments **inside** `aes()`, e.g. for each month different plotting symbols are used and the size of the symbols is dependent on the solar radion. So, multiple legends result.

```{r ggpot-more2var-2, fig.cap="(ref:more2var-2)"}
...
Another option is to use `faceting`; a plot is divided into subplots. [@ggplot2Cheatsheet] Figure \@ref(fig:ggpot-more2var-2) can be transformed by *faceting* to figures \@ref(fig:ggpot-more2var-3) and \@ref(fig:ggpot-more2var-4):

```{r ggpot-more2var-3, fig.cap="(ref:more2var-3)"}
...
```{r ggpot-more2var-4, fig.cap="(ref:more2var-4)"}
...
```{r ggpot-more2var-5, fig.cap="(ref:more2var-5)"}
...
```{r ggpot-more2var-6, fig.cap="(ref:more2var-6)"}
air_ggplot + # call the already defined ggplot
  geom_smooth(method = "lm", se = TRUE) # add a linear model to each subplot
```
...
When a ggplot has been created it can be converted to an interactive plot based on the R package `plolty` [@plotly], meaning that by clicking with the mouse on a plotting symbol its values are shown. In figure \@ref(fig:ggplotly-air-lm) the x- (`Temp`: 74) and y-value (`Wind`: 12.6) are displayed.
...
From figure \@ref(fig:plotly-airquality) it becomes clear that `plot_ly()` opens a new window in which the generated three-dimensional plot can be zoomed in and out, can be shifted within the window and turned around, etc.
